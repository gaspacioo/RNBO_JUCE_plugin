# SimpleMeter (RNBO + JUCE)

Plugin and standalone app built from an RNBO export, using the [RNBO JUCE template](https://github.com/Cycling74/rnbo.example.juce). The current patch provides stereo RMS metering and a `gain` parameter, with a custom webview UI (or optional native UI).

RNBO is part of [Max](https://cycling74.com/max/) by [Cycling '74](https://cycling74.com/). 
...
| --------------------------------- | ------------- |
| export/                           | RNBO C++ export (`rnbo_source.cpp`, `description.json`, …) |
| patches/                          | Your `.maxpat` files (reference only; not compiled) |
| src/                              | Application code and custom UI (`webui/` or `CustomAudioEditor`) |
